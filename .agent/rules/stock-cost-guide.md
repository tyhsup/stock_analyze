...
- 財報分析相關的數值請使用常用商業單位 M/B/T
- 程式碼變數命名上請使用英文
- 技術指標的配色使用強烈的對比色
- 假如有變動變數名稱，需要確認所有有引用的 function 內的變數名稱有更新成新的變數名稱
...
- Implement 2-5 s random delays and rotation of User-Agents.
...
- 收集來的資料儲存在本地端資料庫（例如：MySQL），避免 yfinance 或爬蟲網頁的流量限制
...
- **Logging**: Log SQL execution time for any query taking > 0.5 s.
