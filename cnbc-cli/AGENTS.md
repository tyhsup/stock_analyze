# Cnbc Cli Printed CLI Agent Guide

This directory is a generated `cnbc-cli-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cnbc-cli-pp-cli doctor --json
cnbc-cli-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cnbc-cli-pp-cli which "<capability>" --json
cnbc-cli-pp-cli <command> --help
```
...
```bash
cnbc-cli-pp-cli <command> --help
cnbc-cli-pp-cli <command> --dry-run --agent
```
