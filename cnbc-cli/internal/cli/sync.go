...
Exit codes & warnings:
  ...
  cnbc-cli-pp-cli sync

  # Sync specific resources only
  cnbc-cli-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  cnbc-cli-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  cnbc-cli-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  cnbc-cli-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  cnbc-cli-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/cnbc-cli-pp-cli/data.db)")
