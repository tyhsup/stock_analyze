...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cnbc-cli-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CNBC_CLI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CNBC_CLI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cnbc-cli-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cnbc-cli-pp-cli/feedback.jsonl.
When ` + "`CNBC_CLI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CNBC_CLI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cnbc-cli-pp-cli feedback list
  cnbc-cli-pp-cli feedback list --limit 5
  cnbc-cli-pp-cli feedback list --json`,
