...
Exit codes & warnings:
  ...
  macrotrends-pp-cli sync

  # Sync specific resources only
  macrotrends-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  macrotrends-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  macrotrends-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  macrotrends-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  macrotrends-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/macrotrends-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "macrotrends":
		return db.UpsertMacrotrends(data)
	default:
