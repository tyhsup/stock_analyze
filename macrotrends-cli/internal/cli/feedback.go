...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".macrotrends-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MACROTRENDS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MACROTRENDS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "macrotrends-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.macrotrends-pp-cli/feedback.jsonl.
When ` + "`MACROTRENDS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MACROTRENDS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  macrotrends-pp-cli feedback list
  macrotrends-pp-cli feedback list --limit 5
  macrotrends-pp-cli feedback list --json`,
