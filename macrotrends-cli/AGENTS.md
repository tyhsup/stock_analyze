# Macrotrends Printed CLI Agent Guide

This directory is a generated `macrotrends-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
macrotrends-pp-cli doctor --json
macrotrends-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
macrotrends-pp-cli which "<capability>" --json
macrotrends-pp-cli <command> --help
```
...
```bash
macrotrends-pp-cli <command> --agent
```
...
```bash
macrotrends-pp-cli <command> --help
macrotrends-pp-cli <command> --dry-run --agent
```
