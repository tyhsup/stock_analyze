...
* **ROE 體質檢驗**：優先選擇 ROE > 20% 的標的，代表公司運用股東資金創造獲利的能力強。ROE 決定資本效率與獲利天花板。
...
* **營業利益率與淨利率**：本業獲利結構健全（營益率穩健或擴張），稅後淨利率未受業外匯損或一次性虧損侵蝕。
* **負債比率 (D/E Ratio) 與現金流**：負債比率原則上以 < 50% 為穩健（高資本密集行業另計），自由現金流 (FCF) 必須維持正向流入，排除帳面虛盈實虧陷阱。

| 情境 | ROE | PE | PEG | 基本面品質評等 | 實戰決策 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | 高 (>20%) | 低 | <1 | Strong (優良) | 難得機會：建立部位區（搭配技術面確認） |
| 2 | 高 (>20%) | 中 | ≈1 | Strong (優良) | 體質強韌：分批建立部位區 |
| 3 | 中 (10-20%) | 高 | >1.2 | Moderate (中等) | 觀察區：等待價格回檔修正或基本面獲利提升 |
| 4 | 低 (<10%) | 高 | >1.5 | Weak (弱勢) | 高風險：絕對避開，嚴防價值陷阱 |
...
### 五、 股票分割影響分析標準作業程序 (Stock Split Impact SOP)

股票分割（Stock Split）或反向分割（Reverse Split）會改變流通股數與每股面額/市價，但公司實質總市值與內在價值不變。在進行技術與籌碼分析時，Agent 必須納入以下分割調整 SOP：

#### 1. 價格不連續性與虛假跳空防護 (Price Discontinuity & False Gap Filtering)
* **除權息與分割還原原則**：歷史價格序列、均線與形態頸線必須基於「分割調整後價格（Adjusted Close）」進行計算。若數據源存在未還原跳空，嚴禁將分割跳空缺口判定為趨勢反轉、暴跌破位或崩盤信號。
* **支撐壓力頸線重新校準**：歷史關鍵阻力位/支撐位若在分割日前形成，判讀時必須乘以分割係數（如 1 拆 4 則歷史價位除以 4）以對齊當前價格坐標。

#### 2. 成交量與流動性結構變化 (Volume Expansion & Liquidity Shift)
* **成交股數放大效應**：分割後流通股數倍增且每股單價降低，日常成交量（股數/張數）會產生結構性放大。評估量價關係時，不可將分割後暴增的交易股數直接與分割前絕對股數進行「爆量出貨」的錯誤對比。
* **散戶參與度與籌碼發散風險**：分割通常降低交易門檻，吸引大量散戶資金湧入。Agent 需特別檢視分割後的籌碼集中度：若散戶持股比例急遽上升而大戶/法人順勢逢高減持，需防範「分割利多出盡」的高檔出貨陷阱。

#### 3. 實戰判讀步驟
1. **步驟 1（檢索歷史）**：分析前先確認近期是否有股票分割事件（日期與比例）。
2. **步驟 2（過濾失真）**：檢查形態頸線、均線排列與跳空缺口是否已排除分割干擾。
3. **步驟 3（綜合評價）**：將分割帶來的短期流動性激勵與中長期籌碼穩定度整合於投資建議結論中。
...
```json
{
  ...
  "fundamental_analysis": {
    ...
    "net_margin": 38.2,
    "debt_to_equity": 32.4,
    "free_cash_flow_status": "Positive",
...
| :--- | :--- | :--- | :--- |
| **基本面** | ROE / PEG / 營收 / 獲利比率 | 高 ROE (>20%) + PEG 0.75-1.2 + 月營收成長 + 正向 FCF | ROE 決定資本效率天花板，營益率與淨利率驗證本業品質，負債比與 FCF 防範破產風險 |
...
| **事件面** | 股票分割 (Stock Split) | 檢查近期分割日期與比例，過濾未還原跳空與成交量基期突變 | 避免虛假破位誤判，校準歷史頸線阻力與籌碼流動性 |
...
4. **防範股票分割引發的指標失真陷阱**：在個股經歷股票分割時，必須檢視除權息與分割調整狀態，切忌將未還原之價格斷層誤判為崩跌賣訊，亦不可將分割後股數放大引發之成交量直接視為爆量出貨。
