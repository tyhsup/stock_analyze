# Cnyes Cli Printed CLI Agent Guide

This directory is a generated `cnyes-cli-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cnyes-cli-pp-cli doctor --json
cnyes-cli-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cnyes-cli-pp-cli which "<capability>" --json
cnyes-cli-pp-cli <command> --help
```
...
```bash
cnyes-cli-pp-cli <command> --agent
```
...
```bash
cnyes-cli-pp-cli <command> --help
cnyes-cli-pp-cli <command> --dry-run --agent
```
