...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cnyes-cli-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CNYES_CLI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CNYES_CLI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cnyes-cli-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cnyes-cli-pp-cli/feedback.jsonl.
When ` + "`CNYES_CLI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CNYES_CLI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cnyes-cli-pp-cli feedback list
  cnyes-cli-pp-cli feedback list --limit 5
  cnyes-cli-pp-cli feedback list --json`,
