...
class IntegratedStockPredModel:
    ...
    def generate_gemini_advice(
        ...
        prompt = (
            f"您是專業的金融量化分析師。請綜合分析以下提供的股票（{self.stock_number}）數據，並根據【技術指標協同判讀 SOP】給出權威的投資建議：\n\n"
            f"=== 技術指標協同判讀 SOP (最高分析規範) ===\n"
            f"您在判讀技術面時，必須嚴格遵守以下四層級分析架構與防衛機制：\n"
            f"1. 【層級 1：價格行為與型態（權重 40%）】：價格與 K 線型態優先於一切指標。首先檢視經典反轉型態（頭肩頂/底、雙重頂M/底W）與關鍵頸線防衛價位。\n"
