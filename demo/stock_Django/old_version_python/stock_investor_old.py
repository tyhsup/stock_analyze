...
class stock_investor():
    ...
    def get_data_cycle():
        ...
        for i in range(y_cycle):
            for j in range(m_cycle):
                for k in range(d_cycle):
                    try:
                        ...
                        year = stock_investor.get_year(driver = driver)
            ...
            data.fillna(value = 0)
            mySQL_OP.OP_Fun().upload_all(data, 'stock_tw_analyse', 'stock_investor_data' + '-' + year)
            tds = pd.DataFrame()
        driver.quit()
        
    def update_investor():
        SQL = mySQL_OP.OP_Fun()
        get_date = SQL.get_cost_data('stock_investor', '日期')
        last_date = datetime.strptime(get_date.drop_duplicates().iat[-1,-1],'%Y/%m/%d')
        start_date = datetime.date(last_date + timedelta(days = 1))
        now_date = datetime.today().date()
        url='https://www.twse.com.tw/zh/trading/foreign/t86.html'
        driver=webdriver.Edge()
        driver_wait = WebDriverWait(driver, timeout=20, poll_frequency=1)
        driver.get(url)
        ...
        while start_date <= now_date:
            try:
                ...
                ths = []
                select_e_y = Select(driver.find_element(By.NAME, 'yy'))
                select_e_y.select_by_value(str(start_date.year))
                select_e_m = Select(driver.find_element(By.NAME, 'mm'))
                select_e_m.select_by_value(str(start_date.month))
                select_e_d = Select(driver.find_element(By.NAME, 'dd'))
                select_e_d.select_by_value(str(start_date.day))
                time.sleep(5)
                select_element = Select(driver.find_element(By.NAME, 'selectType'))
                select_element.select_by_value('ALL')
                select_click = driver.find_element(By.CLASS_NAME, 'submit')
                select_click.click()
                time.sleep(5)
                select_page = driver_wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="reports"]/hgroup/div/div[1]/select/option[5]')))
                select_page.click()
                time.sleep(5)
                soup = BeautifulSoup(driver.page_source,"lxml")
                data_table = soup.find('table')
                ...
                data.replace(to_replace = 'None', value = 0)
                data.rename(columns={'證券代號':'number'}, inplace=True)
                SQL.upload_all(data, 'stock_tw_analyse', 'stock_investor')
                driver.refresh()
                time.sleep(10)
    ...
    def update_investor_input_date():
        SQL = mySQL_OP.OP_Fun()
        get_date = SQL.get_cost_data('stock_investor')
        date_input = input('輸入開始日期')
        start_date = datetime.date(datetime.strptime(date_input, '%Y-%m-%d'))
        now_date = datetime.today().date()
        url='https://www.twse.com.tw/zh/trading/foreign/t86.html'
        driver=webdriver.Edge()
        driver_wait = WebDriverWait(driver, timeout=20, poll_frequency=1)
        ...
        while start_date <= now_date:
            try:
                ...
                data = tds.set_axis(ths,axis = 'columns')
                data.rename(columns={'證券代號':'number'}, inplace=True)
                data_new = pd.concat([get_date,data], ignore_index = True)
                data_new = data_new.drop_duplicates()
                data_new.fillna(value = 0)
                data_new.replace(to_replace = '', value = 0)
                data_new.replace(to_replace = 'None', value = 0)
                SQL.upload_all(data_new, 'stock_tw_analyse', 'stock_investor_2')
                driver.refresh()
                time.sleep(5)
                start_date = start_date + timedelta(days = 1)
            except Exception as e :
...
stock_investor.update_investor()
'''SQL = mySQL_OP.OP_Fun()
get_data = SQL.get_cost_data(table_name = 'stock_investor',stock_number = '2330')
pd.set_option("display.max_rows", None)    # 顯示所有列
pd.set_option("display.max_columns", None) # 顯示所有欄
pd.set_option("display.width", None)       # 不自動換行
pd.set_option("display.max_colwidth", None) # 顯示欄位完整內容
print(get_data)'''
