...
#webbug._result_plot()
'''check_data = pd.read_excel('E:/Infinity/webbug/2330_news.xlsx')
check_data['發布時間'] = pd.DatetimeIndex(check_data['發布時間']).to_period('D')
check_data.sort_values(by = '發布時間', ascending = True, inplace = True)
check_data['情緒分析結果'] = check_data['內文'].apply(webbug.GB_analyze_BERT_ver2)
final_data = webbug.GB_percent(check_data)
final_data.to_excel('E:/Infinity/webbug/2330_news_新聞正負分析結果(day).xlsx')'''
#check_data.to_excel('E:/Infinity/webbug/2330_news_情緒分析結果.xlsx')
